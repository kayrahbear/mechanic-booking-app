...
for secret in "${SECRET_NAMES[@]}"; do
  ...
  if gcloud secrets describe "$secret" --project="$PROJECT_ID" &>/dev/null; then
    ...
    if ! terraform state list "module.secrets.google_secret_manager_secret.this[\"$secret\"]" &>/dev/null; then
...
if gcloud secrets describe "calendar-sync-sa" --project="$PROJECT_ID" &>/dev/null; then
  echo "calendar-sync-sa exists in GCP"
  if ! terraform state list "google_secret_manager_secret.calendar_sa_key" &>/dev/null; then
