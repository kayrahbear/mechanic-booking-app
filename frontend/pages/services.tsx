...
const ServicesPage = ({ services }: { services: Service[] }) => {
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-3xl font-bold mb-6 text-gray-900">Our Services</h1>

            {services.length === 0 ? (
                <div className="bg-gray-50 p-6 rounded-lg text-center">
                    <p className="text-gray-800">No services available at the moment.</p>
                </div>
            ) : (
                    ...
                    {services.map((service) => (
                        <div key={service.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            ...
                            <div className="flex justify-between items-center mb-4">
                                <span className="font-medium text-lg text-gray-900">${service.price}</span>
                                <span className="text-gray-700">{service.minutes} minutes</span>
                            </div>
                            <Link
